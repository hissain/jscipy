...
public class SavitzkyGolayFilter {
    ...
    /**
     * Apply Savitzky-Golay filter using 'interp' mode (polynomial fitting at edges).
     *
     ...
     * @param deriv The order of the derivative.
     * @param delta The spacing of the samples.
     * @param pos The position within the window to evaluate (default: center).
     * @return An array of filter coefficients.
     */
    private double[] calculateSavitzkyGolayCoefficients(int windowLength, int polyOrder, int deriv, double delta, int pos) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Singular matrix encountered. Try reducing polyOrder or increasing windowLength.", e);
        }
        RealMatrix pseudoInverse = vTvInv.multiply(vT);

        // The coefficients for the derivative are in the 'deriv'-th row of pseudoInverse
        ...
        // For center position, use direct coefficients (more efficient for standard convolution)
        // The center position logic in Python implementation calculates coefficients for x=0.
        // If pos matches halfWindow, we are evaluating at x=0.
        // The pre-calculated 'coefficientsRow' corresponds to the coefficients for y[0] (or y[deriv])
        // when the polynomial is fit around 0.
        // Wait, the pseudoInverse method gives coefficients C_j such that f(0) = sum(C_j * y_j).
        ...
        // Evaluate the polynomial basis at the specified position to get the effective filter coefficients
        double xPos = pos - halfWindow;
        double[] result = new double[windowLength];

        for (int i = 0; i < windowLength; i++) {
            double sum = 0.0;
            // Sum over polynomial terms: coeffs[i] = sum_k ( P_{deriv, k} * xPos^k * V_{i,k} )?
            // Actually, we want to find weights w_i such that f^(deriv)(xPos) = sum(w_i * y_i).
            ...
            // The 'deriv' row of pseudoInverse gives coefficients to estimate the d-th derivative at x=0 (if the basis is centered at 0).
            ...
            // If `use = 'dot'`, it calculates `(V^T V)^-1 V^T` and takes the row corresponding to `deriv`.
            ...
            */
            // This loop uses `pseudoInverse.getEntry(deriv, k)`. This is fetching the `deriv`-th row?
            ...
            // The Kotlin code used `pseudoInverse.getEntry(deriv, k)`. This is wrong if it iterates k.
            ...
            // Wait, standard SavGol assumes x spacing is 1. If delta != 1, we divide by delta^deriv.
