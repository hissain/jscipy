...
/**
 * Implements Chebyshev Type II filter operations, including `filtfilt` for zero-phase filtering.
 */
public class Chebyshev2Filter {
    ...
    public double[] filtfilt(double[] signal, double sampleRate, double cutoff, int order, double stopBandDb) {
        Chebyshev2 cheby2 = new Chebyshev2();
        cheby2.lowPass(order, stopBandDb, sampleRate, cutoff);

        Biquad[] biquads = cheby2.getBiquads();
    ...
    public double[] filter(double[] signal, double sampleRate, double cutoff, int order, double stopBandDb) {
        Chebyshev2 cheby2 = new Chebyshev2();
        cheby2.lowPass(order, stopBandDb, sampleRate, cutoff);

        double[] output = new double[signal.length];
        for (int i = 0; i < signal.length; i++) {
            output[i] = cheby2.filter(signal[i]);
        }
