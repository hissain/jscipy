...
def generate_fft_test_data(test_id, num_samples):
    ...
    # Save the data
    with open(os.path.join(output_dir, f"fft_input_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, signal_data, fmt='%.18e')
    
    # Save complex FFT result as two columns (real, imag)
    with open(os.path.join(output_dir, f"fft_output_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, np.column_stack((fft_result.real, fft_result.imag)), fmt='%.18e')
    
    # Save complex RFFT result as two columns (real, imag)
    with open(os.path.join(output_dir, f"rfft_output_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, np.column_stack((rfft_result.real, rfft_result.imag)), fmt='%.18e')

    # Save complex IFFT result as two columns (real, imag)
    with open(os.path.join(output_dir, f"ifft_output_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, np.column_stack((ifft_result.real, ifft_result.imag)), fmt='%.18e')

    # Save real IRFFT result
    with open(os.path.join(output_dir, f"irfft_output_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, irfft_result, fmt='%.18e')
...
def generate_stft_test_data(test_id, num_samples, nperseg=256, noverlap=None, nfft=None):
    ...
    # Save input signal
    with open(os.path.join(output_dir, f"stft_input_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, signal_data, fmt='%.18e')
    
    # Save STFT parameters
    with open(os.path.join(output_dir, f"stft_params_{test_id}.txt"), 'w', newline='\n') as f:
    ...
    # Save shape info
    with open(os.path.join(output_dir, f"stft_output_{test_id}_shape.txt"), 'w', newline='\n') as f:
        f.write(f"{stft_shape[0]} {stft_shape[1]}\n")
    
    # Save complex STFT data
    with open(os.path.join(output_dir, f"stft_output_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, np.column_stack((stft_flattened.real, stft_flattened.imag)), fmt='%.18e')
    
    # Save ISTFT result
    with open(os.path.join(output_dir, f"istft_output_{test_id}.txt"), 'w', newline='\n') as f:
        np.savetxt(f, reconstructed, fmt='%.18e')
